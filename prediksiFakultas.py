...
# Fungsi untuk mengambil data dalam database
def fetch_data():
    conn = sqlite3.connect('nilai_siswa.db')
    cursor = conn.cursor()
...
# Fungsi untuk mengupdate baris berdasarkan id
...
# Fungsi untuk memprediksi fakultas berdasarkan 3 nilai
...
# Fungsi untuk submit data yang akan digunakan pada tombol add
def submit():
    try:
        nama = nama_var.get()
...
# Fungsi update yang akan digunakan dalam tombol update
def update():
    try:
...
# Fungsi delete yang akan digunakan dalam tombol delete
def delete():
    try:
        if not selected_record_id.get():
...
# Fungsi untuk membersihkan input yang ada
def clear_inputs():
    nama_var.set("")
    biologi_var.set("")
    fisika_var.set("")
...
# Fungsi untuk mengisi table pada GUI
def populate_table():
...
# Fungsi untuk mengisi form berdasarkan data yang dipilih dari table
